...
public class Main {
		...
		try {
			...
			while( ( line = reader.readLine() ) != null ) {
				if( !line.trim().equals( "" ) ) {
					...
					process( param[0].trim(), Long.parseLong( param[1] ) );
				}
			}
		}
		catch( IOException e ) {
	...
	public static void process( String platApp, long beg ) {
		String begKey = "UUID:" + platApp + ":L";
		String endKey = "UUID:" + platApp + ":R";
		
		try {
			mc.add( begKey, 0, "" + beg ).get();
			mc.add( endKey, 0, "" + beg ).get();
			String begVal = (String) mc.get( begKey );
			String endVal = (String) mc.get( endKey );
			
			logger.info( "Now for {} beg is {} end is {}", platApp, begVal, endVal );
		}
		catch( Exception e ) {
			logger.error( e.getMessage(), e );
		}
