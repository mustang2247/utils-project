...
import static com.hoolai.centersdk.sdk.CenterGateSdk.GAMEID_OLD_POKER;
...
public class TexasScanner extends Scanner {
    ...
    public TexasScanner( String name, List< Integer > rangeIds ) {
        super( name );
...
//                    long vipExp = centerRepo.getVipExp( uid );
                    ...
                    logger.debug( "User:{} Gold:{} Diamond:{} Level:{} VipLevel:{}", uid, gold, 0, lv, 0 );
                    type2tpb.get( 1 ).add( new TopPlayers.TopPlayer( uid, gold, 0, lv, 0, "", "" ) );
    ...
    protected void shutdown() {
        try {
            type2tpb.forEach( ( type, tpb ) -> {
                ...
                logger.info( "Result GameId:{} Type:{} TopData:{}", GAMEID_OLD_POKER, type, JsonUtil.toString( topPlayers ) );
                centerRepo.setGameTopPlayers( GAMEID_OLD_POKER, type, topPlayers );
            } );
