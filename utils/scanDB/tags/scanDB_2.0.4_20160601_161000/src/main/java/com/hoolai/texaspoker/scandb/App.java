...
public class App 
{
	...
	public static TopPlayerList  topPlayerListMahjong;
	public static MinHeap<TopPlayer>  heapMahjong;
    ...
    public static void scanAll( int nthread ) {
		...
		try {
    		...
    		heapMahjong.sort_heap( TOPN );
			client.save( "TOP_PLAYERS_MAHJONG", JsonUtil.toJson( topPlayerListMahjong ) );
    	...
    	while( true ) {
    		...
    		try {
    			...
    			if( basicValue != null && expValue != null ) {
					...
					String appid = UserRepo.getInstance().getAppId( uid );
					if( "1104754063".equals( appid ) ) {
						updateTopPlayerMahjong( player );
					}
    ...
    public static synchronized void updateTopPlayerMahjong( TopPlayer tp ) {
    	topPlayerListMahjong.players[TOPN] = tp;
    	heapMahjong.push_heap( TOPN );
    }
    ...
    private static void init() {
		...
		topPlayerListMahjong = new TopPlayerList( TOPN );
		heapMahjong = new MinHeap<TopPlayer>( topPlayerListMahjong.players );
