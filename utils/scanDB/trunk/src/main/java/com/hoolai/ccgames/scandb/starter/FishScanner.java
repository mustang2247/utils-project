...
public class FishScanner extends Scanner {
    ...
    public AtomicInteger traderIndex = new AtomicInteger( 0 );
    public List< Long > traderUuids = Arrays.asList( 1972888L,
            3478187L,
            534222L,
            4649308L,
            3694728L,
    ...
    private void doWork1( long uid ) {
        try {
            long gold = centerRepo.getNewGold( uid );
            long diamond = centerRepo.getDiamond( uid );
            long exp = centerRepo.getNewExp( uid );
            long vipExp = centerRepo.getNewVipExp( uid );

            logger.debug( "User:{} Gold:{} Diamond:{} Exp:{} VipExp:{}", uid, gold, diamond, exp, vipExp );
            type2tpb.get( 1 ).add( new TopPlayers.TopPlayer( uid, gold, diamond, (int) exp, (int) vipExp, "", "" ) );
            type2tpb.get( 2 ).add( new TopPlayers.TopPlayer( uid, gold, diamond, (int) exp, (int) vipExp, "", "" ) );
            type2tpb.get( 3 ).add( new TopPlayers.TopPlayer( uid, gold, diamond, (int) exp, (int) vipExp, "", "" ) );
            type2tpb.get( 4 ).add( new TopPlayers.TopPlayer( uid, gold, diamond, (int) exp, (int) vipExp, "", "" ) );
        }
        catch( Exception e ) {
    ...
    protected void doWork() {
        ...
        int indexEnd = traderUuids.size();
        while( true ) {
            int index = traderIndex.getAndIncrement();
            if( index >= indexEnd ) break;
            doWork1( traderUuids.get( index ) );
        }
    }
            ...
            type2tpb.forEach( ( type, tpb ) -> {
                ...
                for( int i = 0; i < TOPN; ++i ) {
                    ...
                    if( userId > 0 ) {
                        ...
                        player.level = centerRepo.getNewLevel( userId );
                        player.vipLevel = centerRepo.getVipLevel( userId );
                        topPlayers.add( player );
                    }
                }

                logger.info( "Result GameId:{} Type:{} TopData:{}", GAMEID_FISHING, type, JsonUtil.toString( topPlayers ) );
                boolean rv = centerRepo.setGameTopPlayers( GAMEID_FISHING, type, topPlayers );
                logger.debug( "Save result {}", rv );
            } );
