...
import static com.hoolai.ccgames.scandb.starter.FullScanner.MAHJONG_TOP_KEY;
...
public class TexasScanner extends Scanner {
	...
	protected void doWork() {
		while( true ) {
    		...
    		try {
				...
				long gold = centerRepo.queryGold( uid );
				int lv = centerRepo.queryLevelExp( uid ).getLevel();
	...
	protected void shutdown() {
		try {
			texasTpl.timestamp = System.currentTimeMillis();
			String tplStr = texasTpl.toString();
			logger.info( "{} {}", TEXAS_TOP_KEY, tplStr );

			ExtendedMemcachedClient texasDB = new ExtendedMemcachedClientImpl( "TexasDB", texasCfg.ADDRS );
			texasDB.start();
			texasDB.set( TEXAS_TOP_KEY, tplStr );
